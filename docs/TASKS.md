...
- [x] Quản lý nhân viên:
  ...
  - [x] Tìm kiếm nhân viên theo tên
  - [x] Tìm kiếm nhân viên theo phòng ban
- [x] Quản lý phòng ban
- [x] Đăng ký tài khoản
- [x] Đăng nhập
...
- [x] Thống kê dữ liệu nhân viên:
  - [x] Tổng số nhân viên
  - [x] Số lượng nhân viên theo phòng ban
  - [x] Hiển thị thống kê bằng REST API
  - [x] Hiển thị thống kê bằng Thymeleaf
...
- [x] Thêm các dependency cơ bản:
  ...
  - [x] Thymeleaf
  - [x] Validation
  - [x] Database Driver (H2 dev/test, MySQL prod)
  - [x] Lombok nếu sử dụng (không dùng Lombok — dùng Java records và manual getters)
...
- [x] Chọn một trong:
  - [x] `PasswordEncoder` (BCryptPasswordEncoder được tạo qua @Bean trong InfrastructureConfig)
  - [ ] `ModelMapper` (không dùng — mapping thủ công trong Service)
...
- [x] Có các trường:
  ...
  - [x] `name`
  - [x] `email`
  - [x] `department` (ManyToOne relationship)
